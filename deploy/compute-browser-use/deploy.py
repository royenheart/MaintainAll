...
def _setup_env_impl():
    ...
    if not data.get("ASTRBOT_DASHBOARD_PASSWORD"):
        if q:
            ...
            pwd = click.prompt("AstrBot dashboard password", default="", show_default=False)
            if pwd:
                data["ASTRBOT_DASHBOARD_PASSWORD"] = pwd
            else:
                pwd = secrets.token_urlsafe(12) + "A1"
...
def _install_hermes_connector_plugin():
    ...
    try:
        ...
        if r.returncode == 0:
            click.secho("\n[3/4] Hermes Connector plugin already installed", fg="green")
            return
    except Exception:
        pass  # Container might not be running yet, proceed with install

    click.secho("\n[3/4] Installing Hermes Connector plugin...", fg="yellow")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            click.secho(f"  Install manually in AstrBot Dashboard → Plugins → {PLUGIN_REPO}", fg="yellow")
            return

        # Verify download: check file size and zip integrity
        zip_size = zip_path.stat().st_size
        if zip_size < 1024:
            click.secho(f"  Download corrupted: file too small ({zip_size} bytes)", fg="red")
            click.secho(f"  Install manually in AstrBot Dashboard -> Plugins -> {PLUGIN_REPO}", fg="yellow")
            return
        ...
        for fname in required:
            if not (extracted / fname).is_file():
                click.secho(f"  Plugin package missing required file: {fname}", fg="yellow")
                click.secho(f"  Install manually in AstrBot Dashboard -> Plugins -> {PLUGIN_REPO}", fg="yellow")
                return
        ...
        try:
            subprocess.run(
                ["docker", "cp", str(extracted) + "/.", f"{ASTRBOT_CONTAINER}:{ASTRBOT_PLUGIN_PATH}"],
                check=True, capture_output=True,
            )
            click.secho("  Plugin installed — AstrBot will detect it automatically", fg="green")
            click.secho("  Triggering docker restart to load the plugin...", fg="yellow")
            subprocess.run(
...
def print_service_summary(bind_address: str, has_llm: bool):
    services = [
        ...
        ("Hermes Hub",         8420, "/health",   "AI Agent HTTP wrapper (Hermes CLI + SSE)"),
        ("Hermes Bridge (fallback)", 8421, "/health", "Message forwarding (requires --profile bridge)"),
    ...
    # Hermes Connector plugin hint
    click.echo()
    click.secho("  Hermes Connector (primary AstrBot plugin):", fg="cyan")
    click.echo("    Pre-installed in AstrBot image (Dockerfile RUN git clone)")
    click.echo("    Configure in AstrBot Dashboard → Plugins → Hermes控制器:")
    click.echo(f"      remote_mode: hub")
    click.echo(f"      hub_endpoint: http://hermes-api:8420")
    click.echo(f"      access_token: (from .env HERMES_ACCESS_TOKEN)")
    click.echo("    Features: commands (/hermes ...), approval, progress monitor, SSE events")
    click.echo()
...
def deploy_server(bind_address, interactive=True, reset=False):
    ...
    click.secho(f"\n[1/4] Building images... (Ctrl+C to cancel)", fg="yellow")
    try:
        subprocess.run(["docker", "compose", "build"], cwd=str(SCRIPT_DIR), check=False)
    except KeyboardInterrupt:
        raise

    click.secho(f"\n[2/4] Starting services... (Ctrl+C to cancel)", fg="yellow")
    try:
    ...
    # Install Hermes Connector plugin (skip if already present)
    _install_hermes_connector_plugin()

    click.secho(f"\n[4/4] Waiting for services...", fg="yellow")
    ...
    for attempt in range(6):
        ...
        ok_count = sum(1 for _name, port, _path, _desc in [
            ...
            ("Hermes Hub", 8420, "/health", ""),
        ] if _http_get(_path or "/health", port) == "200")
        if ok_count >= 2:
            break
        if attempt < 5:
            click.echo(f"  Waiting... ({ok_count}/2 services ready, attempt {attempt+1}/6)")
...
def _interactive_deploy_impl():
    ...
    try:
        do_reset = click.confirm("Reset before deploying? (deletes all Docker volumes / data)", default=False)
    except click.exceptions.Abort:
        raise DeployAborted()
    address = interactive_select_address()
    if not address:
