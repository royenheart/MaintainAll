...
def render_profile(st: WizardState) -> str:
    ...
    tun = "true" if st.enable_tun else "false"
...
tun:
  ...
  strict-route: false
  # 不劫持 53：未勾选应用（Chrome 等）继续用系统 DNS / 系统路由出站。
  dns-hijack: []
...
def ask(st: WizardState, installed: bool) -> WizardState:
    ...
    tun = questionary.confirm(
        "打开 TUN？（只让勾选应用走代理；会装虚拟网卡。不劫持系统 DNS。"
        "和 Tailscale/公司 VPN 可能冲突，一般网页不需要开）",
        default=st.enable_tun,
    ).ask()
    if tun is None:
        raise SystemExit(1)
