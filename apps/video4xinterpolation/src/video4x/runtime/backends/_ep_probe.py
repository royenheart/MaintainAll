...
from video4x.runtime.platform import HostPlatform, default_ep_preference, detect_platform
...
def vitisai_cache_key(model_path: Path | str) -> str:
    """Stable unique cache key — never use bare stem (many files share realesrgan_body.onnx)."""
    p = Path(model_path).resolve()
    parts = list(p.parts)
    # Keep last up to 4 path components so fixed/256x256 vs fixed/512x512 differ
    tail = parts[-4:] if len(parts) >= 4 else parts
    key = "_".join(tail).replace(".onnx", "")
    for ch in '<>:"/\\|?*':
        key = key.replace(ch, "_")
    return key[:160] or p.stem


def _vitisai_provider_options(
            ...
            if probe.vitisai:
                ...
                out.append(
                    (
                        ...
                        _vitisai_provider_options(cache_dir, cache_key=cache_key),
                    )
                )
        elif key == "cpu":
            if probe.cpu:
