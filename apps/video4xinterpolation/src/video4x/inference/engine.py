...
from video4x.runtime.backends.base import BackendStats, InterpolationBackend
from video4x.runtime.backends.registry import create_backend, list_backends
from video4x.runtime.preprocess import crop_hw, pack_frame_batch, pad_to_multiple
from video4x.runtime.resolutions import resolve_onnx_paths_for_hw, size_tag
from video4x.runtime.resources import create_resource_sampler
from video4x.runtime.video_io import read_frames, write_video
...
class RifeInferenceEngine:
    ...
    def interpolate_video(
        ...
        Output FPS is ``source_fps * 2`` (2× frame count). If ``fps`` is None,
        source FPS is auto-detected (so 30fps → 60fps, 24fps → 48fps).
        """
        ...
        try:
            ...
            src_fps = float(fps) if fps is not None else detect_video_fps(input_path)
            out_fps = src_fps * 2.0
            tr.emit(
                ProgressEvent(
                    phase="decode",
                    message=f"reading {input_path.name} (src={src_fps:.3g}fps → out={out_fps:.3g}fps)",
                )
            )
            ...
            tr.emit(
                self._progress_base(
                    tr,
                    "encode",
                    message=f"writing {output_path.name} @ {out_fps:.3g}fps",
                    current=len(out_frames),
                    total=len(out_frames),
                )
            )
            write_video(output_path, (f[0] for f in out_frames), fps=out_fps, width=w, height=h)
