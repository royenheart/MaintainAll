...
# Gitea 实例的相关信息
GITEA_HOST = (
    "你的Gitea域名或IP"  # 例如 'gitea.example.com' 或 'services-gitea'（Docker服务名）
)
...
JUPYTERHUB_HOST = (
    "你的JupyterHub域名或IP"  # 例如 'jupyterhub.example.com' 或 'localhost'
)
...
c.GenericOAuthenticator.username_key = (
    "username"  # 设置用户名 key，用于 JupyterHub 识别用户
)
...
c.Authenticator.allow_all = (
    True  # 如果设置为 True，则所有通过 Gitea 认证的用户都可以访问
)
